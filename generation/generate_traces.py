"""
Data-parallel deepconf offline generator (SGLang Engine, no server).

Runs NUM_GPUS SGLang Engine replicas (TP=1) in parallel via multiprocessing,
each pulling (qid, batch_idx) work items from a shared queue so any idle GPU
...
Usage:
  python generation/generate_traces.py --rid deepseek
  python generation/generate_traces.py --rid deepseek --qids 0 1 2
  python generation/generate_traces.py --rid deepseek --num-gpus 4
"""
import os
import json
import time
...
def worker(gpu_id, work_queue, done_queue, prompts_map, gts_map,
    ...
    """Persistent worker: loads SGLang Engine on one GPU, processes batches."""
    os.environ["CUDA_VISIBLE_DEVICES"] = str(gpu_id)
    env_bin = os.path.dirname(os.path.abspath(os.sys.executable))
    os.environ["PATH"] = env_bin + ":" + os.environ.get("PATH", "")

    import sglang as sgl
    from helper import process_output_offline_sglang

    print(f"[GPU {gpu_id}] loading SGLang Engine {model_path}...")
    t0 = time.time()
    engine = sgl.Engine(
        model_path=model_path,
        tp_size=1,
    )
    print(f"[GPU {gpu_id}] engine loaded in {time.time()-t0:.1f}s")
    ...
    while True:
        ...
        traces = []
        for out in outputs:
            traces.append(process_output_offline_sglang(out, gt, WINDOW_SIZE))
...
def main():
    parser = argparse.ArgumentParser(
        description="Data-parallel deepconf offline generator (SGLang Engine)"
    )
